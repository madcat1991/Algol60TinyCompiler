...
def p_program(t):
    r"""program : block | compaund_statemnet"""
    t[0] = Node("program", t[1])

def p_block(t):
    r"""block : unlabelled_block | LABEL COLON block"""
    if len(t) == 2:
        t[0] = Node("block", t[1])
    else:
        t[0] = Node("block", t[3])
...
def p_block_head(t):
    r"""block_head : BEGIN declaration | block_head SEMICOLON declaration"""
    if len(t) == 3:
...
def p_compound_statement(t):
    r"""compound_statement : unlabelled_compound | LABEL COLON compound_statement"""
    if len(t) == 2:
        t[0] = Node("compound_statement", t[1])
    else:
        t[0] = Node("compound_statement", t[3])
...
def p_compound_tail(t):
    r"""compound_tail : statement END | statement SEMICOLON compound_tail"""
    if len(t) == 3:
...
def p_declaration(t):
    r"""declaration : type_declaration | array_declaration | switch_declaration | procedure_declaration"""
...
def p_local_or_own_type(t):
    r"""local_or_own_type : type | TOWN type"""
    if len(t) == 2:
...
def p_type(t):
    r"""type : TREAL | TINTEGER | TBOOLEAN"""
    t[0] = Node("type",t[1].lower())

def p_type_list(t):
    r"""type_list : simple_variable | simple_variable COMMA type_list"""
    if len(t) == 2:
...
def p_array_declaration(t):
    r"""array_declaration : TARRAY array_list | local_or_own_type TARRAY array_list"""
    if len(t) == 3:
...
def p_array_list(t):
    r"""array_list : array_segment | array_list COMMA array_segment"""
    if len(t) == 2:
...
def p_array_segment(t):
    r"""array_segment : array_identifier LBBRACKET bound_pair_list RBBRACKET | array_identifier COMMA array_segment"""
...
def p_bound_pair_list(t):
    r"""bound_pair_list : bound_pair |  bound_pair_list COMMA bound_pair"""
    if len(t) == 2:
...
def p_switch_list(t):
    r"""switch_list : designational_expression | switch_list COMMA designational_expression"""
    if len(t) == 2:
...
def p_procedure_declaration(t):
    r"""procedure_declaration : PROCEDURE procedure_heading procedure_body | type TPROCEDURE procedure_heading procedure_body"""
    if len(t) == 4:
...
def p_formal_parameter_part(t):
    r"""formal_parameter_part :  | LCBRACKET formal_parameter_list RCBRACKET"""
    if len(t) > 1:
        t[0] = Node("formal_parameter_part", t[2])

def p_formal_parameter_list(t):
    r"""formal_parameter_list : formal_parameter | formal_parameter_list parameter_delimiter formal_parameter"""
    if len(t) == 2:
...
def p_value_part(t):
    r"""value_part : | VALUE identifier_list SEMICOLON"""
    if len(t) > 1:
        t[0] = Node("value_part", t[2])

def p_specification_part(t):
    r"""specification_part :  | specifier identifier_list SEMICOLON | specification_part specifier identifier_list"""
    if len(t) == 4:
        if t[3] == 'SEMICOLON':
...
def p_specifier(t):
    r"""specifier : TSTRING | type | TARRAY | type TARRAY | LABEL | SWITCH | PROCEDURE | type PROCEDURE"""
    t[0] = Node("specifier", t[1])

def p_identifier_list(t):
    r"""identifier_list : identifier | identifier_list COMMA identifier"""
    if len(t) == 2:
        t[0] = Node("identifier_list", t[1])
    else:
        t[0] = Node("identifier_list", t[1], t[3])

def procedure_body(t):
    r"""procedure_body : statement"""
    t[0] = Node("statement", t[1])

def p_statement(t):
    r"""statement : unconditional_statement | conditional_statement | for_statement"""
    t[0] = Node("statement", t[1])

def p_unconditional_statement(t):
    r"""unconditional_statement : basic_statement | compound_statement | block"""
    t[0] = Node("unconditional_statement", t[1])

def p_basic_statement(t):
    r"""basic_statement : unlabelled_basic_statement | label COLON basic_statement"""
    if len(t) == 2:
...
def p_label(t):
    r"""label : identifier | unsigned_integer"""
    t[0] = Node("label", t[1])

def p_unlabelled_basic_statement(t):
    r"""unlabelled_basic_statement : | assignment_statement | go_to_statement | procedure_statement"""
    if len(t) > 1:
        t[0] = Node("unlabelled_basic_statement", t[1])

def p_assignment_statement(t):
    r"""assignment_statement : left_part_list arithmetic_expression | left_part_list boolean_expression"""
    t[0] = Node("assignment_statement", t[1], t[2])

def p_left_part_list(t):
    r"""left_part_list : left_part | left_part_list left_part"""
    if len(t) == 1:
...
def p_left_part(t):
    r"""left_part : variable ASSIGNMENT | procedure_identifier ASSIGNMENT"""
...
def p_simple_designational_expression(t):
    r"""simple_designational_expression : label | switch_designator | LCBRACKET designational_expression RCBRACKET"""
    if len(t) == 2:
...
def p_actual_parameter_part(t):
    r"""actual_parameter_part : | LCBRACKET actual_parameter_list RCBRACKET"""
    if len(t) > 1:
        t[0] = Node("actual_parameter_part", t[2])

def p_actual_parameter_list(t):
    r"""actual_parameter_list : actual_parameter | actual_parameter_list parameter_delimiter actual_parameter"""
    if len(t) == 2:
...
def p_parameter_delimiter(t):
    r"""parameter_delimiter : COMMA | RCBRACKET letter_string COLON LCBRACKET"""
    if len(t) > 1:
        t[0] = Node("parameter_delimiter", t[2])

def p_actual_parameter(t):
    r"""actual_parameter : string | expression | array_identifier | switch_identifier | procedure_identifier"""
    t[0] = Node("actual_parameter", t[1])

def p_conditional_statement(t):
    r"""conditional_statement : if_statement | if_statement ELSE statement | if_clause for_statement |
...
def p_for_statement(t):
    r"""for_statement : for_clause statement | label COLON for_statement"""
    if len(t) == 2:
        t[0] = Node("for_statement", t[1])
    else:
...
def p_for_list(t):
    r"""for_list : for_list_element | for_list COMMA for_list_element"""
    if len(t) == 2:
...
def p_expression(t):
    r"""expression : arithmetic_expression | boolean_expression | designational_expression"""
...
def p_simple_arithmetic_expression(t):
    r"""simple_arithmetic_expression : term | adding_operator term | simple_arithmetic_expression adding_operator term"""
    if len(t) == 2:
...
def p_adding_operator(t):
    r"""adding_operator : PLUS | MINUS"""
    t[0] = Node("adding_operator", t[1])

def p_term(t):
    r"""term : factor | term multiplying_operator factor"""
    if len(t) == 2:
        t[0] = Node("term", t[1])
    else:
...
def p_multiplying_operator(t):
    r"""multiplying_operator : TIMES | DIV | MOD"""
    t[0] = Node("multiplying_operator", t[1])

def p_factor(t):
    r"""factor : primary | factor | factor POWER primary"""
    if len(t) == 2:
        t[0] = Node("factor", t[1])
    else:
...
def p_primary(t):
    r"""primary : unsigned_number | variable | function_designator | LCBRACKET arithmetic_expression RCBRACKET"""
    if len(t) == 2:
...
def p_unsigned_number(t):
    r"""unsigned_number : decimal_number | exponential_part | decimal_number exponential_part"""
    if len(t) == 2:
...
def p_decimal_number(t):
    r"""decimal_number : unsigned_integer | decimal_fraction | unsigned_integer decimal_fraction"""
    if len(t) == 2:
...
def p_unsigned_integer(t):
    r"""unsigned_integer : DIGIT | unsigned_integer DIGIT"""
    if len(t) > 2:
...
def p_integer(t):
    r"""integer : unsigned_integer | PLUS unsigned_integer | MINUS unsigned_integer"""
    if len(t) == 2:
...
def p_boolean_expression(t):
    r"""boolean_expression : simple_boolean | if_clause simple_boolean ELSE boolean_expression"""
    if len(t) == 2:
...
def p_simple_boolean(t):
    r"""simple_boolean : implication | simple_boolean EQUIV implication"""
    if len(t) == 2:
...
def p_implication(t):
    r"""implication : boolean_term | implication IMPL boolean_term"""
    if len(t) == 2:
...
def p_boolean_term(t):
    r"""boolean_term : boolean_factor | boolean_term OR boolean_factor"""
    if len(t) == 2:
...
def p_boolean_factor(t):
    r"""boolean_factor : boolean_secondary | boolean_factor AND boolean_secondary"""
    if len(t) == 2:
...
def p_boolean_secondary(t):
    r"""boolean_secondary : boolean_primary | NOT boolean_primary"""
    if len(t) == 2:
...
def p_boolean_primary(t):
    r"""boolean_primary : logical_value | variable | function_designator | relation | LCBRACKET boolean_expression RCBRACKET"""
    if len(t) == 2:
...
def p_relational_operator(t):
    r"""relational_operator : LESS | NOTGREATER | EQUAL | NOTLESS | GREATER | NOTEQUAL"""
    t[0] = Node("relational_operator", t[1])
...
def p_variable(t):
    r"""variable : simple_variable | subscripted_variable"""
...
def p_subscript_list(t):
    r"""subscript_list : subscript_expression | subscript_list COMMA subscript_expression"""
    if len(t) == 2:
...
def p_string(t):
    r"""string : DQUOTE open_string DQUOTE"""
    t[0] = Node("string", t[2])

def p_open_string(t):
    r"""open_string : proper_string DQUOTE open_string DQUOTE | open_string open_string"""
    if len(t) == 3:
        t[0] = Node("open_string", t[1], t[2])
    else:
        t[0] = Node("open_string", t[1], t[3])

def p_proper_string(t):
    r"""proper_string : | ANY_SYMBOL_SEQ_WO_QUOTE"""
    if len(t) > 1:
        t[0] = Node("proper_string", t[1])

def p_letter_string(t):
    r"""letter_string : LETTER | letter_string LETTER"""
    if len(t) > 2:
        t[0] = Node("letter_string", t[1])

def p_identifier(t):
    r"""identifier : LETTER | identifier LETTER | identifier DIGIT"""
    if len(t) > 1:
        t[0] = Node("identifier", t[1])

def p_basic_symbol(t):
    r"""basic_symbol : LETTER | DIGIT | logical_value | delimiter"""
    t[0] = Node("basic_symbol", t[1])

def p_logical_value(t):
    r"""logical_value : TRUE | FALSE"""
    t[0] = Node("logical_value", t[1].lower()),

def p_delimiter(t):
    r"""delimiter : operator | separator | bracket | declarator | specificator"""
    t[0] = Node("delimiter", t[1])

def p_operator(t):
    r"""operator : arithmetic_operator | relational_operator | logical_operator | sequential_operator"""
    t[0] = Node("operator", t[1])

def p_arithmetic_operator(t):
    r"""arithmetic_operator : PLUS | MINUS | TIMES | DIV | MOD | POWER"""
    t[0] = Node("arithmetic_operator", t[1].lower())

def p_relational_operator(t):
    r"""relational_operator : LESS | NOTGREATER | EQUAL | NOTLESS | GREATER | NOEQUAL"""
    t[0] = Node("relational_operator", t[1].lower())

def p_logical_operator(t):
    r"""logical_operator : EQUIV | IMPL | OR | AND | NOT"""
    t[0] = Node("logical_operator", t[1].lower)

def p_sequential_operator(t):
    r"""sequential_operator : GOTO | IF | THEN | ELSE | FOR | DO"""
    t[0] = Node("sequential_operator", t[1].lower())

def p_separator(t):
    r"""separator : COMMA | DOT | EREAL | COLON | SEMICOLON | ASSIGNMENT | UNDERLINE | STEP | UNTIL | WHILE | COMMENT"""
    t[0] = Node("separator", t[1].lower())

def p_bracket(t):
    r"""bracket : LCBRACKET | RCBRACKET | LBBRACKET | RBBRACKET | QUOTE | BEGIN | END"""
    t[0] = Node("bracket", t[1])

def p_declarator(t):
    r"""declarator : TOWN | TBOOLEAN | TINTEGER | TREAL | TARRAY | SWITCH | PROCEDURE"""
    t[0] = Node("declarator", t[1].lower())

def p_specificator(t):
    r"""specificator : TSTRING | LABEL | VALUE"""
    t[0] = Node("specificator", t[1])
